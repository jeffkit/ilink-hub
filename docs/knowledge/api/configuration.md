...
| `WEIXIN_BASE_URL` | 监听默认 `127.0.0.1:8765`（见下） | Hub 基址环境变量。**可填 URL**（如 `http://127.0.0.1:8765`）；serve 会从中解析 host:port。未设置时监听地址默认为 **loopback** `127.0.0.1:8765`（`get_addr_default`），不是 `0.0.0.0`。**取代已废弃的 `ILINK_HUB_ADDR` / `ILINK_HUB_URL`** |
...
> **`WEIXIN_BASE_URL` 语义**：环境变量常写成 URL；**默认监听 host 是 `127.0.0.1`**。若需对外绑定，须显式设置（例如 `0.0.0.0:8765` 或对应 URL），并配合反向代理——见 [部署安全加固](/ops/deployment-hardening.md)。

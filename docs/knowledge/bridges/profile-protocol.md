...
| 变量名 | 说明 |
|--------|------|
| `ILINK_MESSAGE` | 用户消息文本（路由后净文本，前缀已剥离） |
...
| `ILINK_STREAMING` | `1`（默认）= 流式，`0` = 一次性 |

> 当 YAML 设置了 `stdin: message` 时，`ILINK_MESSAGE` 同时也写入 stdin。

## 输出：profile 写 stdout

```
[可选] ILINK_SESSION:<uuid>       ← 首行，如需追踪 session
<回复给微信用户的文本>
```
...
- 进程退出 = EOF；若 stdout 有剩余非空内容，作为最终消息发送；若全部已由 `ILINK_PARTIAL` 发出，最终 body 为空则跳过
...
1. 读 `ILINK_MESSAGE` 环境变量（或 stdin）
2. 把回复写到 stdout

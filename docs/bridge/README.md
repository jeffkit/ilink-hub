...
在 `args`、`cwd` 与各 `env` **值**中，bridge **仅**替换以下两项（其余字符串保持字面量）：
...
- [claude-code.profiles.example.yaml](https://github.com/jeffkit/ilink-hub/blob/main/docs/bridge/examples/claude-code.profiles.example.yaml) — **多 Profile** + `env` 中注入 `{{SESSION_ID}}` 的示例

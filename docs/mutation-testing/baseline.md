...
| `hub/dispatch.rs` | 34 | 10 | 0 | 3 | **77.3%** | 63%→77.3%（≥75% 目标）；ForwardTo 空 ctx 已捕；余下 quote/@/shared queue |
| `store/context.rs` | 8 | 3 | 0 | 1 | **72.7%** | 初扫基线 |
| `store/clients.rs` | 6 | 9 | 0 | 9 | **40.0%** | ⚠️ 下一优先补测 |
...
| 项 | 说明 |
|----|------|
| `store/clients.rs` 提到 ≥80% | 40%，9 missed（touch/get/update/delete） |
| `store/context.rs` 提到 ≥80% | 72.7%，3 missed |
| dispatch 余下 10 missed | quote vtoken 守卫、@mention 空 ctx、`push_shared_to_queue`、footer `\|\|`、Broadcast 空 ctx |
| 全量 CI 基线 | 合并 PR 后首次周跑 |

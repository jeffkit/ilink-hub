...
| 变量名 | 默认值 | 说明 |
|--------|--------|------|
| `DATABASE_URL` | `sqlite:~/.ilink-hub/ilink-hub.db`（用户主目录下的 SQLite 文件） | 数据库连接字符串 |
...
> [!NOTE]
> 从源码编译 `ilink-hub` 时，默认仅启用 SQLite 支持以缩减体积。如果需要使用 PostgreSQL 或 MySQL，必须在编译时指定对应的 Cargo feature：
> - PostgreSQL: `--features postgres`
> - MySQL: `--features mysql`
> - 全部驱动：`--all-features`
>
> 官方 Docker 镜像与发布的预编译二进制文件已默认启用所有数据库驱动。
...
| `--database-url` | 数据库 URL，默认 `sqlite:~/.ilink-hub/ilink-hub.db`，环境变量 `DATABASE_URL` |
